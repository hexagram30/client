...
use crate::game;
use crate::map;
use rltk::{Point, RGB, Rltk, Console};
use specs;
use specs::prelude::*;
...
pub fn draw(ecs: &World, ctx: &mut Rltk) {
    ...
    ctx.draw_box(0, gui.map_area.height - 1, gui.width - 1, gui.text_area.height, RGB::named(rltk::WHITE), RGB::named(rltk::BLACK));
    ...
    for (_player, stats) in (&players, &combat_stats).join() {
        ...
        ctx.print_color(12, gui.map_area.height - 1, RGB::named(rltk::YELLOW), RGB::named(rltk::BLACK), &health);
        // XXX let's put the 28 and 51 into config ... need to look up API usage
        ctx.draw_bar_horizontal(28, gui.map_area.height - 1, 51, stats.hp, stats.max_hp, RGB::named(rltk::GREEN), RGB::named(rltk::BLACK));
    }
